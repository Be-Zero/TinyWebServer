...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    ...
    if (text[0] == '\0')
    {
        /*如果HTTP请求有消息体，则还需要读取m_content_length字节的消息体，状态机 转移到CHECK_STATE_CONTENT状态*/
...
/*当得到一个完整、正确的HTTP请求时，我们就分析目标文件的属性。如果目标文件存 在、对所有用户可读，且不是目录，则使用mmap将其映射到内存地址m_file_address 处，并告诉调用者获取文件成功*/
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        if (temp <= -1)
        {
            /*如果TCP写缓冲没有空间，则等待下一轮EPOLLOUT事件。虽然在此期间，服务器无 法立即接收到同一客户的下一个请求，但这可以保证连接的完整性*/
            if (errno == EAGAIN)
            {
...
bool http_conn::add_response(const char *format, ...)
{
    ...
    int len = vsnprintf(m_write_buf + m_write_idx, WRITE_BUFFER_SIZE - 1m_write_idx, format, arg_list);
